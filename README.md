...
- **メーター（5時間制限 / 週間・全モデル / 週間・Fable）— 無料で取れるのはターミナルの `claude` だけ**: Claude Code がステータスラインに渡す `rate_limits` を `usage-latest.json`（このフォルダ内、`.gitignore` 済み）に書き出して読みます。ステータスラインはローカル実行で API トークンを消費しません（公式ドキュメント明記）。`~/.claude/settings.json`:
  ...
  - Pro / Max のみ。**重要な制約**: ステータスラインの `rate_limits` は「そのターミナルの `claude` が最後に受け取った API 応答」の値で、サーバーに問い合わせ直すわけではありません。ファイルは 60 秒ごとに書かれますが、そのセッションで何も送らなければ値は変わらず、他のセッションの消費は反映されません。ボードは「値の更新 HH:MM」に値が最後に変わった時刻を出し、10 分以上変化が無ければ「以後 API 応答なし」と表示します。正確な今の % が必要なときは、そのターミナルで何か 1 つ送ってください（その分のトークンは消費します）
